...
True_EM_mat <- function(fBeta, feat_matrix, XH1, XH0) {
  ...
  retdf <- data.frame(opf) %>%
    select(-value.objfn) %>%
    mutate(Intercept = opf$par[1], feature = cn) %>%
    rename(Beta = par)
  if (nrow(retdf) > 1) {
...
sim_dat <- function(tprior, ngenes, samplesize, betap) {
  ...
  return(data_frame(
    Z = Z, x = x, XH1 = XH1, XH0 = XH0, BF = BF, tprior = c(tprior),
...
comp_model <- function(tprior, betavec, betap, ngenes, samplesize, A, i) {
  ...
  return(data_frame(
    Intercept = betavec[1], Beta = betavec[2], Intercept_t = tfvec[1],
...
gen_est <- function(tprior, betap, ngenes, samplesize, A, i, marg = FALSE) {
  ...
  if (marg) {
    retdfl <- list()
    for (j in 2:(ncol(A))) {
      fbeta <- stats::coefficients(glm(sample_data$tmu ~ A[, c(1, j)] + 0, family = quasibinomial(link = "logit")))
      retdfl[[j]] <- EM_mat(Beta0 = fbeta, feat_mat = A[, c(1, j)], BF = BF)
    }
    retdf <- dplyr::bind_rows(retdfl)
  } else {
    fbeta <- stats::coefficients(stats::glm(sample_data$tmu ~ A + 0, family = stats::quasibinomial(link = "logit")))
    retdf <- EM_mat(Beta0 = fbeta, feat_mat = A, BF = BF)
  }
