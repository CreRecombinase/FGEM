
#include <eigen3/Eigen/Core>
//#include <RcppEigen.h>
#include <cmath>
#include <cstddef>
#include <limits>
//[[Rcpp::depends(RcppEigen)]]
//using namespace Rcpp;

// template<typename VecTypeX, typename MatTypeX>
// class GlmBase
// {
// protected:

//     const int nvars;      // dimension of beta
//     const int nobs;       // number of rows

//     VecTypeX beta;        // parameters to be optimized
//     VecTypeX beta_prev;   // auxiliary parameters

//     VecTypeX eta;
//     VecTypeX var_mu;
//     VecTypeX mu_eta;
//     VecTypeX mu;
//     VecTypeX z;
//     VecTypeX w;
//     MatTypeX vcov;
//     VecTypeX se;
//     double dev, devold, devnull;

//     int maxit;            // max iterations
//     double tol;           // tolerance for convergence
//     bool conv;


//     virtual bool converged()
//     {
//         if (std::abs(dev - devold)/(0.1 + std::abs(dev)) < tol)
//         {
//             return true;
//         } else
//         {
//             return false;
//         }
//     }


//     virtual void update_eta()
//     {

//     }

//     virtual void update_var_mu()
//     {

//     }

//     virtual void update_mu_eta()
//     {

//     }

//     virtual void update_mu()
//     {

//     }

//     virtual void update_z()
//     {

//     }

//     virtual void update_w()
//     {

//     }

//     virtual void step_halve()
//     {

//     }

//     virtual void run_step_halving(int &iterr)
//     {

//     }

//     virtual void update_dev_resids()
//     {

//     }

//     virtual void update_dev_resids_dont_update_old()
//     {

//     }

//     virtual void solve_wls(int iter)
//     {

//     }

//     virtual void save_se()
//     {

//     }


// public:
//     GlmBase(int n_, int p_,
//             double tol_ = 1e-6,
//             int maxit_ = 100) :
//     nvars(p_), nobs(n_),
//     beta(p_),
//     beta_prev(p_), // allocate space but do not set values
//     eta(n_),
//     var_mu(n_),
//     mu_eta(n_),
//     mu(n_),
//     z(n_),
//     w(n_),
//     vcov(p_, p_),
//     se(p_),
//     maxit(maxit_),
//     tol(tol_)
//     {}

//     virtual ~GlmBase() {}

//     virtual void init_parms(const Map<VecTypeX> & start_,
//                             const Map<VecTypeX> & mu_,
//                             const Map<VecTypeX> & eta_) {}

//     void update_beta()
//     {
//         //VecTypeX newbeta(nvars);
//         next_beta(beta);
//         //beta.swap(newbeta);
//     }

//     int solve(int maxit)
//     {
//         int i;

//         conv = false;



//     virtual VecTypeX get_beta()     { return beta; }
//     virtual VecTypeX get_eta()      { return eta; }
//     virtual VecTypeX get_se()       { return se; }
//     virtual VecTypeX get_mu()       { return mu; }
//     virtual VecTypeX get_weights()  { return w; }
//     virtual VecTypeX get_w()        { return w.array().square(); }
//     virtual double get_dev()        { return dev; }
//     virtual int get_rank()          { return nvars; }
//     virtual MatTypeX get_vcov()     { return vcov; }
//     virtual bool get_converged()    { return conv; }

// };



// double FGEM_Logit_log_lik_cpp(const Rcpp::NumericVector &Beta, const Rcpp::NumericMatrix &x, const Rcpp::NumericVector B){

//   // arma::vec pvec = 1/(1+arma::exp(-(x*Beta)));
//   // Rcpp::Rcout<<"pvec:"<<pvec<<std::endl;

// //  arma::vec uvec = (pvec%B)/((pvec%B)+(1-pvec));
//   double sum=0;
//   const size_t p=B.size();
//   for(int i=0; i<p; i++){


//   }
//   // Rcpp::Rcout<<"uvec:"<<uvec<<std::endl;
//   return(arma::sum(log(pvec%B+(1-pvec))));
// }

double x_d_opx(double x) { return x / (1 + x); }


Eigen::ArrayXd logit_link(const Eigen::ArrayXd& mu)
{
  return mu.unaryExpr([](double x) { return std::log(x/(1+x)); });
}

Eigen::ArrayXd logit_linkinv(const Eigen::ArrayXd eta){

  constexpr double THRESH = 30.;
  constexpr double MTHRESH = -30.;
  constexpr double INVEPS = 1/std::numeric_limits<double>::epsilon();
  constexpr double DOUBLE_EPS = std::numeric_limits<double>::epsilon();
  return eta.unaryExpr([&](double etai){
                         auto tmp = (etai < MTHRESH) ? DOUBLE_EPS :
                           ((etai > THRESH) ? INVEPS : exp(etai));
                         return tmp/(1+tmp);
                       });
}





// Eigen::ArrayXd fast_logit(Eigen::ArrayXd y,Eigen::MatrixXd X){

//   const size_t n= y.size();
//   const size_t p=X.cols();

//   Eigen::ArrayXd weights = Eigen::ArrayXd::Constant(n,1);
//   Eigen::ArrayXd offset = Eigen::ArrayXd::Zero(n);
//   Eigen::ArrayXd mustart = (weights * y + 0.5)/(weights + 1);
//   Eigen::ArrayXd m= weights * y;
//   Eigen::ArrayXd eta = logit_link(mustart);
//   Eigen::ArrayXd mu = logit_linkinv(eta);
//   Eigen::ArrayXd start = Eigen::ArrayXd::Zero(p);



//   for(int i = 0; i < maxit; ++i)
//     {

//       //        update_var_mu();
//       //        NumericVector var_mu_nv = variance_fun(mu);
//       //        std::copy(var_mu_nv.begin(), var_mu_nv.end(), var_mu.data());

//       //    update_mu_eta()
//       //     NumericVector mu_eta_nv = mu_eta_fun(eta);
//       // std::copy(mu_eta_nv.begin(), mu_eta_nv.end(), mu_eta.data());

//       // update_z();
//       // virtual void update_z()
//       // {
//       //   // z <- (eta - offset)[good] + (y - mu)[good]/mu.eta.val[good]
//       //   z = (eta - offset).array() + (Y - mu).array() / mu_eta.array();
//       // }

//       //      update_w();
//       //       w = (weights.array() * mu_eta.array().square() / var_mu.array()).array().sqrt();

//       Model.solve_wls(i);

//       update_eta();

//       //update_mu();
//       //NumericVector mu_nv = linkinv(eta);
//       //std::copy(mu_nv.begin(), mu_nv.end(), mu.data());

//       update_dev_resids();

//       run_step_halving(i);

//       if (std::isinf(dev) && i == 0)
//         {
//           Rcpp::stop("cannot find valid starting values: please specify some");
//         }

//       if(converged())
//         {
//           conv = true;
//           break;
//         }


//     }

//   save_se();

//   //return std::min(i + 1, maxit);


// }
